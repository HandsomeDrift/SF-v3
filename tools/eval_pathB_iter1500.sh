...
# Run AFTER inference completes (540/540 mp4s in results/alpha_540/pathB_p1_iter1500/).
# Single GPU, ~10 min.
set -euo pipefail
...
OUT_DIR=${RESULTS}/pathB_p1_iter1500
SUMMARY_1500=${RESULTS}/summary_pathB_p1_iter1500.json
SUMMARY_8WAY=${RESULTS}/summary_8way_pathB.json

cd ${PROJ}

# pre-flight
count=$(ls "${OUT_DIR}"/*.mp4 2>/dev/null | wc -l)
echo "=== pathB_p1_iter1500: ${count}/540 mp4s ==="
...
rows = [("E0_new_code","baseline"),("E3_cosine","Path A cosine"),("E4_reverse","Path A winner"),
        ...
        ("pathB_p1_iter1500","Path B sweet spot (NEW)")]
...
# diff vs iter 2000
b1500 = e.get("pathB_p1_iter1500"); b2000 = e.get("pathB_p1_iter2000")
if b1500 and b2000:
