#!/usr/bin/env bash
# Path B P1 iter 1500 - 540 sample 8-way inference on gpu5.
# Goal: verify diagnostic claim that iter 1500 FVD << iter 2000 FVD 517.
# val_loss 0.396 (iter 1500) vs 0.486 (iter 2000) predicts ~425 FVD.
...
NAME=pathB_p1_iter1500
OUTDIR=results/alpha_540/${NAME}

cd $PROJ
rm -rf ${OUTDIR}
mkdir -p ${OUTDIR} logs
: > logs/pathB_iter1500_8way.pids
...
for gpu in 0 1 2 3 4 5 6 7; do
    split=${gpu}
    log=logs/pathB_iter1500_gpu5_gpu${gpu}.log
    port=$((29800 + gpu))
    ...
    CUDA_VISIBLE_DEVICES=${gpu} \
      nohup ${PY} -m torch.distributed.run \
          ...
          > ${log} 2>&1 &
    echo "${NAME} gpu5 gpu${gpu} split${split} pid=$!" >> logs/pathB_iter1500_8way.pids
done

echo "=== launched 8 processes on gpu5 ==="
cat logs/pathB_iter1500_8way.pids
echo ""
echo "Monitor with:"
echo "  ssh gpu5 \"ls ${OUTDIR}/*.mp4 2>/dev/null | wc -l\"  # expect 540 at completion"
echo "  tail -f logs/pathB_iter1500_gpu5_gpu0.log"
